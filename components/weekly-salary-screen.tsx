...
export function WeeklySalaryScreen() {
    ...
    const fetchWeeklySalary = async () => {
      ...
      try {
        const response = await fetch(
          `http://3.109.152.136/api/payrolls/getWeeklySalary?fromDate=${weekRange.start}&toDate=${weekRange.end}`,
          {
            headers: {
  ...
  const handleConfirmGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://3.109.152.136/api/payrolls/GenerateWeeklySalary?anyDateInWeek=${dateString}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${auth.token}`,
