...
export function WeeklySalaryScreen() {
    ...
    const fetchWeeklySalary = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/api/payrolls/getWeeklySalary?fromDate=${weekRange.start}&toDate=${weekRange.end}`,
          {
            headers: {
  ...
  const handleConfirmGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/payrolls/GenerateWeeklySalary?anyDateInWeek=${dateString}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${auth.token}`,
