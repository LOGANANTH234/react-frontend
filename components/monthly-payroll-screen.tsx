...
export function MonthlyPayrollScreen() {
    ...
    const fetchMonthlyPayrollData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://3.109.152.136:3000/api/payrolls/getMonthlySalary?month=${selectedMonth}`,
          {
            headers: {
  ...
  const handleConfirmGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://3.109.152.136:3000/api/payrolls/generateMonthlyPayroll?month=${selectedMonth}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${auth.token}`,
